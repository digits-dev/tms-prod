<?php namespace App\Http\Controllers;
	...
	class AdminCompaniesController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col = [];
            $this->col[] = ["label"=>"Company ID","name"=>"company_code"];
			...
			$this->form = [];
            $this->form[] = ['label'=>'Company ID','name'=>'company_code','type'=>'text','validation'=>'required|min:1|max:50','width'=>'col-sm-6'];
			$this->form[] = ['label'=>'Company Name','name'=>'company_name','type'=>'text','validation'=>'required|min:1|max:150','width'=>'col-sm-6'];
	    ...
	    public function actionButtonSelected($id_selected,$button_name) {
            ...
            switch ($button_name) {
                case 'set_status_ACTIVE':
                    Company::whereIn('id',$id_selected)->update([
                        'status'=>'ACTIVE',
	    ...
	    */
        ...
        public function getPosCompany()
        {
            $dataCompany = DB::connection('mysql_tunnel')
                ->table('sm_company')
                ->where('factive_flag',1)
                ->get();

            foreach ($dataCompany as $key => $value) {
                //check if w3p activated
                $w3p = self::getW3pAccount($value->fcompanyid);

                if(!empty($w3p)){
                    Company::firstOrCreate([
                        'company_code' => $value->fcompanyid
                    ],[
                        'company_code' => $value->fcompanyid,
                        'company_name' => strtoupper($value->fname)
                    ]);
                }
        ...
        public function getW3pAccount($company)
        {
            return DB::connection('mysql_tunnel')
                ->table('w3p_sm_user')
