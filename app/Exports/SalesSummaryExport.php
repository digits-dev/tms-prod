...
class SalesSummaryExport implements FromQuery, WithHeadings, WithMapping
{
    ...
    public function map($sales): array {
        return [
            ...
            $sales->cashier_name,
            $sales->serviced_by,
            $sales->total_line_value+$sales->senior_discount+$sales->pwd_discount+$sales->dip_discount+$sales->service_charge-$sales->total_discount,
            $sales->gross_amount,
    ...
    public function query()
    {
        $sales = SalesReport::query()
            ...
            ->select('sales_reports.sales_trx_date',
                ...
                DB::raw('SUM(sales_report_lines.total_line_value) as total_line_value'));
        ...
        return $sales->groupBy('sales_reports.id','companies.company_name','source_types.source_name','pos_accounts.account_name','pos_branches.branch_name');
    }
}
